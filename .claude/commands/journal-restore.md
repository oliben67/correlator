...
`.catalyst-proj/CODE-OF-CONDUCT.md` §9, `.catalyst-proj/rules/Rules-of-Rules.md` §12.
Input: $ARGUMENTS

1. If `<timestamp>` is missing, ask for it.
2. Read `.catalyst-proj/development/journal.jsonl`. For every file path
...
3. Materialize each into a new side directory (e.g.
   `.catalyst-proj/.journal-restore/<timestamp>/`) via
